...
uniform float4 _WireColor = float4(0.0, 1.0, 1.0, 1.0);
uniform float4 _WireSecondaryColor = float4(1.0, 0.0, 1.0, 1.0);
...
void geom(triangle v2g i[3], inout TriangleStream<g2f> triangleStream)
{
    ...
    o.projPos = ComputeScreenPos(o.projectionSpaceVertex);
...
fixed4 frag(g2f i) : SV_Target
{
    ...
    fixed4 wColor = lerp(_WireColor, _WireSecondaryColor, Linear01Depth(tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(i.projPos)).r));
    
    fixed4 finalColor = lerp(_BaseColor, wColor, t);
//finalColor=tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(i.projPos));
    return finalColor;
}